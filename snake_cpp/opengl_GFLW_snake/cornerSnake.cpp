...
GLuint headTexture, bodyTexture, tailTexture, cornerTexture, fruitTexture;
...
void Draw(GLFWwindow* window) {
    ...
    for (int i = 0; i < nTail; i++) {
        bool isCorner = (tailDirection[i] != tailDirection[i -1]);
        float cornerXPos = (float)tailX[i] / width * 2 - 1 + cellWidth / 2;
        float cornerYPos = (float)tailY[i] / height * 2 - 1 + cellHeight / 2;
        glBegin(GL_QUADS);
        switch (tailDirection[i]) {
            case LEFT:
                glTexCoord2f(1, 0); glVertex2f(cornerXPos - cellWidth / 2, cornerYPos - cellHeight / 2);
                glTexCoord2f(1, 1); glVertex2f(cornerXPos + cellWidth / 2, cornerYPos - cellHeight / 2);
                glTexCoord2f(0, 1); glVertex2f(cornerXPos + cellWidth / 2, cornerYPos + cellHeight / 2);
                glTexCoord2f(0, 0); glVertex2f(cornerXPos - cellWidth / 2, cornerYPos + cellHeight / 2);
                break;
            case RIGHT:
                glTexCoord2f(0, 1); glVertex2f(cornerXPos - cellWidth / 2, cornerYPos - cellHeight / 2);
                glTexCoord2f(0, 0); glVertex2f(cornerXPos + cellWidth / 2, cornerYPos - cellHeight / 2);
...
int main() {
    ...
    cornerTexture = loadTexture("./textures/corner.png");
